...
export default function CorporateOperatorImportPage() {
    ...
    const downloadTemplate = () => {
        ...
        const example = "Operario 1,operario1@empresa.com,Pass123!\nOperario 2,operario2@empresa.com,Pass123!"
    ...
    return (
        <div className="max-w-4xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
            <div className="flex items-center gap-4 text-white">
                <Link href="/corporate/operators" className="p-2 hover:bg-white/10 rounded-full transition-colors">
                    <ArrowLeft className="h-6 w-6" />
                </Link>
                <div>
                    <h1 className="text-3xl font-black italic uppercase bg-gradient-to-r from-indigo-400 to-indigo-600 bg-clip-text text-transparent">Importar Operarios</h1>
                    <p className="text-gray-400 font-medium">Carga masivamente tu equipo de seguridad y logística.</p>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 text-white">
                <div className="bg-white/[0.03] border border-white/10 rounded-[2.5rem] p-8 space-y-6">
                    ...
                    >
                        <FileText className={cn("h-12 w-12", file ? "text-indigo-400" : "text-gray-600")} />
                        <div className="text-center">
                            <p className="text-sm font-bold text-white mb-1">
                                {file ? file.name : "Selecciona tu archivo CSV"}
                            </p>
                            <p className="text-xs text-gray-500 font-mono tracking-tighter">Máximo 5MB • Formato .csv</p>
                        </div>
                        <input
                            type="file"
                            accept=".csv"
                    ...
                    {!result ? (
                        <div className="bg-white/[0.03] border border-white/10 rounded-[2.5rem] p-8 space-y-4 shadow-2xl">
                            ...
                            <ul className="space-y-3 text-sm text-gray-400 leading-relaxed font-medium">
                                <li className="flex gap-2">
                                    ...
                                    Encabezados: name, email, password.
                                </li>
                                <li className="flex gap-2">
                                    <span className="text-indigo-400 font-bold">•</span>
                                    Los correos deben ser únicos en todo el sistema.
                                </li>
                                <li className="flex gap-2">
                                    <span className="text-indigo-400 font-bold">•</span>
                                    Si no incluyes contraseña, se asignará "Operario123!".
                                </li>
                                <li className="flex gap-2">
                                    <span className="text-indigo-400 font-bold">•</span>
                                    Fila de ejemplo: "Juan Lopez,juan@corp.com,PassABC"
                                </li>
                            </ul>
                        </div>
                    ) : (
                        <div className="bg-white/[0.03] border border-white/10 rounded-[2.5rem] p-8 space-y-6 animate-in zoom-in-95 duration-500 shadow-2xl">
                            ...
                            <div className="p-6 bg-green-500/10 border border-green-500/20 rounded-3xl">
                                ...
                                <p className="text-xs text-green-400/70 font-bold uppercase tracking-widest">Operarios registrados con éxito</p>
                            </div>

                            {result.errors.length > 0 && (
                                <div className="space-y-3">
                                    <p className="text-sm font-bold text-red-400 font-mono tracking-tighter">Log de Errores ({result.errors.length}):</p>
                                    <div className="max-h-40 overflow-auto space-y-2 pr-2 scrollbar-thin scrollbar-thumb-red-500/20">
                                        {result.errors.map((err, i) => (
                                            <p key={i} className="text-xs text-gray-500 bg-red-500/5 p-3 rounded-xl border border-red-500/10 font-medium">
                                                {err}
                                            </p>
                                        ))}
                                    </div>
                            ...
                            <button
                                onClick={() => router.push("/corporate/operators")}
